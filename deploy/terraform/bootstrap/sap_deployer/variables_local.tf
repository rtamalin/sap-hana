locals {
  ...
  environment = lower(try(local.infrastructure.environment, ""))
  location    = try(local.infrastructure.region, "")
  codename    = lower(try(local.infrastructure.codename, ""))

  // Management vnet
  vnet_mgmt        = try(local.infrastructure.vnets.management, {})
  ...
  vnet_mgmt_name = local.vnet_mgmt_exists ? split("/", local.vnet_mgmt_arm_id)[8] : (local.vnet_mgmt.name != "" ? local.vnet_mgmt.name : "DEP00")
  ...
  deployer_vm_count = length(local.deployers)

  enable_deployer_public_ip = try(local.options.enable_deployer_public_ip, false)
}
