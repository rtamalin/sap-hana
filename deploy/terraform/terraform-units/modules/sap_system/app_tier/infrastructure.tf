...
resource "azurerm_lb_rule" "scs" {
  ...
  resource_group_name            = var.resource_group[0].name
  loadbalancer_id                = azurerm_lb.scs[0].id
  name                           = format("%s%s%s%05d-%02d", local.prefix, var.naming.separator, local.resource_suffixes.scs_scs_rule, 0, count.index)
  protocol                       = "All"
  frontend_port                  = 0
  backend_port                   = 0
...
resource "azurerm_lb_rule" "ers" {
  ...
  resource_group_name            = var.resource_group[0].name
  loadbalancer_id                = azurerm_lb.scs[0].id
  name                           = format("%s%s%s%05d-%02d", local.prefix, var.naming.separator, local.resource_suffixes.scs_ers_rule, 0, count.index)
  protocol                       = "All"
  frontend_port                  = 0
  backend_port                   = 0
...
resource "azurerm_lb_rule" "web" {
  ...
  resource_group_name            = var.resource_group[0].name
  loadbalancer_id                = azurerm_lb.web[0].id
  name                           = format("%s%s%s%05d-%02d", local.prefix, var.naming.separator, local.resource_suffixes.web_alb_inrule, 0, count.index)
  protocol                       = "All"
  frontend_port                  = 0
  backend_port                   = 0
